...
def load_and_preprocess_data():
    ...
    return cpu, gpu, req, bench, cpu_mc, gpu_scores

def prepare_training_data(bench, cpu_mc, gpu_scores, req):
...
def train_model(progress_callback=None):
    """Main training function that can be called from API"""
    try:
        if progress_callback:
            progress_callback(5, "Loading and preprocessing data...")
        
        # Load and preprocess data
        cpu, gpu, req, bench, cpu_mc, gpu_scores = load_and_preprocess_data()
        
        if progress_callback:
            progress_callback(15, "Preparing training data...")
        
        # Prepare training data
        df = prepare_training_data(bench, cpu_mc, gpu_scores, req)
        
        if progress_callback:
            progress_callback(25, "Engineering features...")
        ...
        if progress_callback:
            progress_callback(35, "Removing outliers...")

        # Outlier removal
        df_train = remove_outliers(df_train, 'FPS', factor=2.0)
        print(f"Training data size after outlier removal: {len(df_train)} rows")

        if progress_callback:
            progress_callback(45, "Selecting features...")
        ...
        if progress_callback:
            progress_callback(55, "Splitting data...")
        ...
        if progress_callback:
            progress_callback(75, "Hyperparameter tuning...")
        ...
        # Save to database
        save_model_stats_to_db(cpu, gpu, bench, req, df_train, mae, rmse, r2, cv_scores, selected_features)
        ...
        if progress_callback:
            progress_callback(-1, f"Training failed: {str(e)}")
        return {
            'success': False,
            'error': str(e)
        }
...
def save_model_stats_to_db(cpu, gpu, bench, req, df_train, mae, rmse, r2, cv_scores, selected_features):
